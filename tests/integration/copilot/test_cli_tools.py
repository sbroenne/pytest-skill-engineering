...
class TestCLIOperations:
    ...
    async def test_run_python_script(self, copilot_eval, tmp_path):
        """Eval can create and run a Python script via CLI."""
        agent = CopilotEval(
            name="cli-runner",
        ...
        # Eval should have used terminal (may be called run_in_terminal or powershell)
    ...
    async def test_use_git_cli(self, copilot_eval, tmp_path):
        """Eval can use git CLI to initialize a repository."""
        agent = CopilotEval(
            name="git-operator",
    ...
    async def test_cli_tool_output_used_in_response(self, copilot_eval, tmp_path):
        """Eval reads CLI output and incorporates it into its response."""
