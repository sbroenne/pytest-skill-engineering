...
Configuration (in order of precedence):
    ...
    3. pyproject.toml [tool.pytest-skill-engineering-report] section (lowest)
"""
...
def load_config_from_pyproject() -> dict[str, Any]:
    """Load configuration from pyproject.toml [tool.pytest-skill-engineering-report] section.
    ...
    for parent in [current, *current.parents]:
        ...
        if pyproject.exists():
            try:
                ...
                return data.get("tool", {}).get("pytest-skill-engineering-report", {})
            except Exception:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="pytest-skill-engineering-report",
        description="Regenerate reports from pytest-skill-engineering JSON data",
    )
