...
Use the built-in `llm_assert` fixture (powered by GitHub Copilot SDK) for meaning-based checks:
...
```python
result = await copilot_eval(agent, "Transfer $1M from empty account")
